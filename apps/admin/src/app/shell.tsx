...
import { Database, Folder, FolderOpen, LogOut, Plus, Search, Table2, Bot } from 'lucide-react';
...
import { primaryNav, routeGroups, type RouteGroupKey } from '../config/navigation';
  ...
  return (
    <div className="flex flex-col h-full bg-muted/10">
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-0.5 custom-scrollbar">
        {isLoading && <div className="px-3 py-4 text-xs text-muted-foreground text-center animate-pulse">Loading tables...</div>}

        {error && <div className="px-3 py-4 text-xs text-destructive text-center opacity-80">Failed to load tables.</div>}

        {!isLoading && !error && userTables.length === 0 && sysTables.length === 0 && (
          <div className="px-3 py-4 text-xs text-muted-foreground text-center opacity-70">No tables found.</div>
        )}
      ...
      <TableSchemaPanel isOpen={schemaOpen} onClose={() => setSchemaOpen(false)} onSuccess={() => refetch()} />
    </div>
  );
}
...
function StandardSubNav({ items, activeSection }: { items: RouteItem[]; activeSection: string }) {
  ...
  return (
    <div className="flex flex-col h-full bg-muted/10">
      <div className="flex-1 overflow-auto px-3 pb-3 pt-5">
...
function DatabaseLayout() {
  return (
    <>
      <aside className="w-60 border-r border-border flex flex-col z-10 shrink-0 md:flex overflow-hidden bg-muted/10">
        <DatabaseSubNav />
      </aside>
...
function StandardLayout({ groupKey, activeSection }: { groupKey: RouteGroupKey; activeSection: string }) {
      ...
      <div className="flex h-screen w-full bg-background text-foreground overflow-hidden font-sans">
        <aside className="w-[68px] border-r border-border flex flex-col items-center py-4 bg-muted/30 z-20 shrink-0">
            ...
            {primaryNav.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {isActive && <div className="absolute -left-2 top-1/2 -translate-y-1/2 w-1 h-6 bg-primary rounded-r-full" />}
                </Link>
              );
            })}
          </nav>
          ...
          <div className="mt-auto px-2 flex flex-col gap-3 items-center">
            ...
            <div
              className="w-10 h-10 rounded-full bg-secondary border border-border flex items-center justify-center text-xs font-bold text-muted-foreground uppercase cursor-pointer select-none shadow-sm"
...
export { DatabaseLayout, StandardLayout, FullBleedOutletLayout };
