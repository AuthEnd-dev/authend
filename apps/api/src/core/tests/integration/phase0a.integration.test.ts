  ...
  describe('Phase 0A integration hardening', () => {
    ...
    test('password reset requests and reset completion work without SMTP', async () => {
      ...
      const { captured } = await withCapturedInfoLogs(() =>
        Promise.resolve(
          app.request(`${appUrl}/api/auth/request-password-reset`, {
            method: 'POST',
            headers: jsonHeaders(),
            body: JSON.stringify({
    ...
    test('email verification send and verify flow works without SMTP', async () => {
      ...
      const { captured } = await withCapturedInfoLogs(() =>
        Promise.resolve(
          app.request(`${appUrl}/api/auth/send-verification-email`, {
            method: 'POST',
            headers: jsonHeaders(),
            body: JSON.stringify({
    ...
    test('magic link plugin sends a sign-in link and creates a session after verification', async () => {
      ...
      const { captured } = await withCapturedInfoLogs(() =>
        Promise.resolve(
          app.request(`${appUrl}/api/auth/sign-in/magic-link`, {
            method: 'POST',
            headers: jsonHeaders(),
            body: JSON.stringify({
    ...
    test('session data traffic is rate limited by session id', async () => {
      const user = await createUserSession('session.ratelimit@authend.test', 'Session Rate Limit');

      await withApiRateLimitSettings(
        {
          sessionRateLimitPerMinute: 2,
        },
        async () => {
          const firstResponse = await app.request(`${appUrl}/api/data/articles`, {
            headers: {
          ...
          const otherUser = await createUserSession('session.ratelimit.other@authend.test', 'Session Rate Limit Other');
          const otherResponse = await app.request(`${appUrl}/api/data/articles`, {
            headers: {
              origin: appUrl,
              cookie: otherUser.jar.toHeader(),
            },
