...
import { assertSafeIdentifier, ensureFieldDefaults, validateDraft, withDefaultId } from '@authend/shared'
...
import { applySqlMigration, persistGeneratedSchemaMigration, previewGeneratedSchemaMigration } from './migration-service'
...
import { sql } from "drizzle-orm";`
  ...
  const enums = draft.tables.map(withDefaultId).flatMap((table) =>
    table.fields
      .map(ensureFieldDefaults)
      .filter((field) => field.type === 'enum' && field.enumValues?.length)
      .map(
        (field) =>
          `export const ${enumVariableName(table.name, field.name)} = pgEnum("${table.name}_${field.name}_enum", [${field.enumValues!.map((value) => JSON.stringify(value)).join(', ')}]);`,
      ),
  )
...
${withId.fields.map((field) => `  ${field.name}: ${drizzleColumnForTable(withId.name, field, referenceAccessor)},`).join('\n')}
...
function sortNewTablesForCreation (tables: TableBlueprint[]) {
  ...
  const ready = tables.map((table) => table.name).filter((name) => (remainingDependencies.get(name)?.size ?? 0) === 0)
...
async function readLiveSchemaState (tableNames: string[]) {
  ...
  const enumTypeNames = Array.from(
    new Set(columns.filter((column) => column.dataType === 'USER-DEFINED').map((column) => column.udtName)),
  )
  const enumRows =
...
export async function getSchemaDriftReport (rawDraft?: SchemaDraft) {
  ...
  for (const table of generatedTables) {
    ...
    for (const field of table.fields.map(ensureFieldDefaults)) {
      ...
      if (field.unique) {
        const hasUniqueIndex = (indexesByTable.get(table.name) ?? []).some(
          (index) => index.unique && index.columns.length === 1 && index.columns[0] === field.name,
        )
    ...
    for (const compoundIndex of table.indexes) {
      const hasIndex = (indexesByTable.get(table.name) ?? []).some(
        (index) =>
          index.columns.length === compoundIndex.length &&
          index.columns.every((column, indexPosition) => column === compoundIndex[indexPosition]),
      )
      if (!hasIndex) {
...
${withId.fields.map((field) => `  ${columnSql(withId.name, field, withId.primaryKey)}`).join(',\n')}
  ...
  for (const columns of withId.indexes) {
    ...
    statements.push(
      `create index if not exists "${indexName}" on "${withId.name}" (${columns.map((column) => `"${column}"`).join(', ')});`,
    )
  }
...
exception
  when duplicate_object then null;
      ...
      if (field.indexed && !field.unique && field.name !== nextTable.primaryKey) {
        statements.push(
          `create index if not exists "${indexName(nextTable.name, field.name)}" on "${nextTable.name}" ("${field.name}");`,
        )
      }
      continue
    }
  ...
  const baseDraft = {
    tables: tables.map((table) => {
      ...
      return {
        ...
        fields: fields.filter((field) => field.tableId === table.id).map((field) => field.definition as FieldBlueprint),
      }
    }),
...
export async function applyDraft (rawDraft: SchemaDraft, actorUserId?: string | null) {
  ...
  const droppedTableNames = current.tables.map((table) => table.name).filter((tableName) => !nextTableNames.has(tableName))
